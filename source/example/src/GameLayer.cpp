...
void GameLayer::loadTestTexture() noexcept
{
  ...
  if(!useHostImageCopy)
  {
    ...
    mVulkanContext->executeImmediateCommand([&](const vk::CommandBuffer& commandBuffer) -> void {
      /* Barrier | Undefined to TransferDst */ {
        ...
        const auto dependencyInfo = vk::DependencyInfo().setImageMemoryBarriers(imageTransferDst);
      ...
      mTestTexture->generateMipmaps(commandBuffer,
                                    ImageState{vk::ImageLayout::eTransferDstOptimal, vk::AccessFlagBits2::eTransferWrite,
  ...
  else
  {
    ...
    mVulkanContext->executeImmediateCommand([&](const vk::CommandBuffer& commandBuffer) -> void {
      // ! This leaves the image in ShaderReadOnly layout
      mTestTexture->generateMipmaps(commandBuffer, ImageState{vk::ImageLayout::eShaderReadOnlyOptimal},
                                    ImageState{vk::ImageLayout::eShaderReadOnlyOptimal,
                                               vk::AccessFlagBits2::eShaderRead | vk::AccessFlagBits2::eShaderSampledRead,
                                               vk::PipelineStageFlagBits2::eAllGraphics});
    });
...
#pragma region
  ...
  device->getHandle().updateDescriptorSets(write, {});
#pragma endregion

  // 5. Free texture data memory when done.
  texture.free();

  spdlog::debug("Loaded texture: {} [HostImageCopy={}]", texture.fileName, STYLE_YN(useHostImageCopy));
}
