...
namespace ptvc::rhi
{
    ...
    Descriptor::~Descriptor()
    {
        ...
        if (const auto result = device.freeDescriptorSets(mDescriptorPool, mSetCount, mSets.data());
    ...
    {
        /* DescriptorPool */ {
            const auto poolSizes = mLayoutBindings
                | std::views::transform([&](const auto& b) {
                    return vk::DescriptorPoolSize()
                        .setDescriptorCount(b.descriptorCount * mSetCount)
                        .setType(b.descriptorType);
                })
