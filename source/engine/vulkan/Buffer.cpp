...
#define check_is_host_visible(msg) \
    if (!mHostVisible) { exitWithError("{} (not host visible)", msg); }

// Buffer bounds check
#define check_bounds(msg, testSize, realSize) \
    if (testSize > realSize) { exitWithError("{} (Size {} out of bounds for Buffer of size {})", msg, testSize, realSize); }
...
namespace ptvc::rhi
{
    Result<SPtr<Buffer>> Buffer::create(const BufferCreateInfo& createInfo) noexcept
    {
        ...
        if (createInfo.size == 0)
        {
            return std::unexpected(std::format(
                "Failed to create Buffer: Invalid size {}", createInfo.size));
        }
    ...
    void Buffer::setData(const void* pData, const vk::DeviceSize size, const vk::DeviceSize offset) const noexcept
    {
        ...
        if (const auto result = vmaCopyMemoryToAllocation(mDevice->getAllocator(), pData, mAllocation, offset, size);
            result != VK_SUCCESS)
        {
    ...
    void Buffer::readBack(void* pData, const vk::DeviceSize size, const vk::DeviceSize offset) const noexcept
    {
        ...
        if (const auto result = vmaCopyAllocationToMemory(mDevice->getAllocator(), mAllocation, offset, pData, size);
            result != VK_SUCCESS)
        {
    ...
    Result<void> Buffer::createBuffer(const BufferCreateInfo& createInfo) noexcept
    {
        ...
        auto bufferInfo = vk::BufferCreateInfo()
            ...
            .setUsage(createInfo.usageFlags | eTransferSrc | eTransferDst | eShaderDeviceAddress);
        ...
        const auto result = vmaCreateBuffer(mDevice->getAllocator(), reinterpret_cast<VkBufferCreateInfo*>(&bufferInfo),
            &allocInfo, reinterpret_cast<VkBuffer*>(&mBuffer),
            &mAllocation, &mAllocationInfo);
