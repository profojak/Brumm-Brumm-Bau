...
void Image::generateMipmaps(const vk::CommandBuffer&         commandBuffer,
                            const std::optional<ImageState>& srcState,
  ...
  /* Transition all mip levels to dstLayout layout */ {
    const auto barrier_End =
        vk::ImageMemoryBarrier2()
            ...
            .setDstAccessMask(dstState.has_value() ? dstState->accessMask :
...
{
  const auto usageFlags = createInfo.usageFlags | vk::ImageUsageFlagBits::eTransferDst | vk::ImageUsageFlagBits::eTransferSrc;
