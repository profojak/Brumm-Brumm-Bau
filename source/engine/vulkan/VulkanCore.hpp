...
#define VK_CHECK(RESULT)                                                                                               \
  if(RESULT != vk::Result::eSuccess)                                                                                   \
  {                                                                                                                    \
    return exitWithError("Error: {}", STYLE_ERROR(to_string(RESULT)));                                                 \
  }
...
/**
     ...
     */
template <class T>
[[nodiscard]] Result<bool, std::vector<const char*>> evaluateFeatureSupport(const std::vector<T>& availableFeatures,
                                                                            const std::vector<const char*>& requiredFeatures) noexcept
{
  static_assert(std::is_same_v<vk::LayerProperties, T> || std::is_same_v<vk::ExtensionProperties, T>,
                "T must be either Layer or ExtensionProperties");
