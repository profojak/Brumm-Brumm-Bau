...
[[nodiscard]] static Option<uint32_t> findQueueFamilyIndex(const std::vector<vk::QueueFamilyProperties2>& queueFamiliesProperties,
                                                           const vk::QueueFlags queueFlags)
{
  ...
  for(uint32_t i = 0; i < queueFamiliesProperties.size(); i++)
  {
    ...
    if(static_cast<uint32_t>(queueFamiliesProperties[i].queueFamilyProperties.queueFlags & queueFlags)
       == static_cast<uint32_t>(queueFlags))
    {
...
[[nodiscard]] static bool checkHostImageCopySupport(const vk::PhysicalDevice& physicalDevice) noexcept
{
  ...
  auto                               features2 = vk::PhysicalDeviceFeatures2().setPNext(&vulkan14Features);
...
#pragma region "Device and Vulkan Core 1.x Features"
  ...
  auto vulkan_1_2     = platform::getPhysicalDeviceVulkan12Features().setPNext(&vulkan_1_1);
...
#pragma endregion
  ...
  const auto graphicsQueueInfo = vk::DeviceQueueInfo2().setQueueFamilyIndex(graphicQueueIndex).setQueueIndex(0);

  mGraphicsQueue = {.familyProperties = queueFamilyProperties[graphicQueueIndex],
