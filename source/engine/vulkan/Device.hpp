...
namespace ptvc::rhi
{
    ...
    public:
        ...
        [[nodiscard]] bool isHostImageCopyAvailable() const noexcept
        {
            return mIsHostImageCopyAvailable;
        }
    ...
    void Device::setLabel(const ObjectNameInfo<T>& objectNameInfo) const
    {
        ...
        const std::string_view name = objectNameInfo.name.empty()
            ? "Unknown" : objectNameInfo.name;
