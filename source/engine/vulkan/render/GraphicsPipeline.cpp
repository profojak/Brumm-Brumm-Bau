...
{
  return vk::PipelineDynamicStateCreateInfo().setDynamicStateCount(0).setPDynamicStates(nullptr).setPNext(nullptr);
}
...
GraphicsPipelineBuilder& GraphicsPipelineBuilder::addAttachment(const vk::Format format,
                                                                const vk::PipelineColorBlendAttachmentState& state) noexcept
{
...
UPtr<Pipeline> GraphicsPipelineBuilder::create(const SPtr<Device>& device) noexcept
{
  ...
  const auto layoutCreateInfo =
      vk::PipelineLayoutCreateInfo().setSetLayouts(mDescriptorSetLayouts).setPushConstantRanges(mPushConstantRanges);
  ...
  const auto shaderStageInfos =
      shaders | std::views::transform([](const auto& x) { return x.shaderStageInfo; }) | std::ranges::to<std::vector>();
