...
namespace ptvc::rhi
{
    ...
    void Pipeline::pushConstant(const void* pData, const vk::CommandBuffer& commandBuffer,
        const uint32_t pcRangeIndex) const noexcept
    {
        if (pcRangeIndex >= mPushConstantRanges.size())
        {
            exitWithError("Invalid PushConstantRange index: {}, the Pipeline ({}) was created with {} range(s).",
                pcRangeIndex, "TODO_LABEL" /* mLabel */, mPushConstantRanges.size());
        }
