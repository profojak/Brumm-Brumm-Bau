...
static vk::Bool32 VKAPI_CALL debugMessengerCallback(const vk::DebugUtilsMessageSeverityFlagBitsEXT           severity,
    ...
    : mWindow(createInfo.window)
{
  ...
  spdlog::debug("GPU ({}) Extension support:\n{}", detail::styledPhysicalDeviceName(mPhysicalDeviceProperties),
                mDeviceExtensions.toString());
  ...
  /* CommandPool */ {
    const auto commandPoolInfo =
        vk::CommandPoolCreateInfo().setQueueFamilyIndex(mDevice->getGraphicsQueue().familyIndex).setFlags(vk::CommandPoolCreateFlagBits::eTransient);
    try
    {
...
Result<Frame> VulkanContext::beginFrame() const noexcept
{
  ...
  const auto allocInfo =
      vk::CommandBufferAllocateInfo().setCommandPool(mCommandPool).setCommandBufferCount(1).setLevel(vk::CommandBufferLevel::ePrimary);
...
void VulkanContext::endFrame_submitAndPresent(const Frame& frame) const noexcept
{
  ...
  const auto commandBufferSubmitInfo = vk::CommandBufferSubmitInfo().setCommandBuffer(frame.commandBuffer);

  const auto waitSemaphoreInfo =
      vk::SemaphoreSubmitInfo().setSemaphore(frame.sImageAvailable).setStageMask(vk::PipelineStageFlagBits2::eAllCommands);

  const auto signalSemaphoreInfo =
      vk::SemaphoreSubmitInfo().setSemaphore(frame.sRenderingFinished).setStageMask(vk::PipelineStageFlagBits2::eAllCommands);
  ...
  mDevice->getGraphicsQueue().getHandle(0).submit2(vkSubmitInfo, frame.fPresentFinished);
  ...
  const auto result = mDevice->getGraphicsQueue().getHandle(0).presentKHR(presentInfo);
...
void VulkanContext::executeImmediateCommand(const std::function<void(const vk::CommandBuffer&)>& lambda) const noexcept
{
  const auto allocateInfo =
      vk::CommandBufferAllocateInfo().setLevel(vk::CommandBufferLevel::ePrimary).setCommandPool(mCommandPool).setCommandBufferCount(1);
...
void VulkanContext::rebuildSwapchain() noexcept
{
  ...
  auto       swapchainResult = Swapchain::create({mDevice, mWindow, mSurface, oldSwapchain.get()});
...
{
  constexpr auto applicationInfo =
      vk::ApplicationInfo().setApiVersion(VK_API_VERSION_1_4).setPApplicationName("ptvc-project").setPEngineName("ptvc-runtime");
  ...
  {
    exitWithError("The following required Vulkan Instance extensions are missing: {}", join(extensionSupportResult.error(), ", "));
  }
  ...
  {
    exitWithError("The following required Vulkan Instance layers are missing: {}", join(layerSupportResult.error(), ", "));
  }
...
Result<vk::PhysicalDevice> VulkanContext::selectPhysicalDevice() const noexcept
{
  ...
  for(const auto& physicalDevice : mInstance.enumeratePhysicalDevices())
  {
    ...
    availableDeviceNames.push_back(std::format("{}[Type={}, Score={}]", std::string(properties.deviceName.data()),
                                               to_string(properties.deviceType), scores[physicalDevice]));
  }
  ...
  return std::ranges::max_element(scores, [](const auto& a, const auto& b) -> bool { return a.second < b.second; })->first;
}
