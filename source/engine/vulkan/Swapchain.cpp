...
namespace ptvc::rhi
{
    namespace detail
    {
        ...
        [[nodiscard]] static Result<vk::SurfaceFormatKHR>
        getSurfaceFormat(const vk::PhysicalDevice physicalDevice, const vk::SurfaceKHR surface) noexcept
        {
            ...
            {
                if (fmt.format == gPreferredFormat &&
                    fmt.colorSpace == gPreferredColorSpace)
                {
                    return fmt;
                }
            }
        ...
        [[nodiscard]] static Result<vk::PresentModeKHR>
        getPresentMode(const vk::PhysicalDevice physicalDevice, const vk::SurfaceKHR surface) noexcept
        {
            ...
            return contains(supportedModes, gPreferredPresentMode)
                ? gPreferredPresentMode
                : supportedModes[0];
        }

        // Ensure the extent is within the supported bounds.
        [[nodiscard]] static vk::Extent2D
        getSwapchainExtent(const IWindow* pWindow, const vk::SurfaceCapabilitiesKHR& surfaceCapabilities) noexcept
        {
            vk::Extent2D extent;
    ...
    Result<UPtr<Swapchain>> Swapchain::create(const SwapchainCreateInfo& createInfo) noexcept
    {
        ...
        spdlog::debug("Created new Swapchain [w={}, h={}, imageCount={}, format={}]",
            swapchain->mExtent.width, swapchain->mExtent.height, swapchain->mImageCount, vk::to_string(swapchain->mFormat));
