...
void Geometry::init(const rhi::VulkanContext* pRHI)
{
  ...
  /* Vertex Buffer */ {
    auto result = rhi::Buffer::create({.size        = vertexSize,
  ...
  /* Index Buffer */ {
    auto result = rhi::Buffer::create({.size        = indexSize,
  ...
  pRHI->executeImmediateCommand([&](const vk::CommandBuffer& commandBuffer) -> void {
    ...
    const auto vertexCopy =
        vk::CopyBufferInfo2().setSrcBuffer(staging.value()->getHandle()).setDstBuffer(mVertexBuffer->getHandle()).setRegions(vertexRegion);
    ...
    const auto indexCopy =
        vk::CopyBufferInfo2().setSrcBuffer(staging.value()->getHandle()).setDstBuffer(mIndexBuffer->getHandle()).setRegions(indexRegion);
